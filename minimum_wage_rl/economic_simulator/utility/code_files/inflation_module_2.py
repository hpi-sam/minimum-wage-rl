...
def set_product_price_and_quantity(emp_worker_list, unemp_worker_list, country, metrics):
    ...
    # lambda result, worker: result + (worker.worker_account_balance - (worker.worker_account_balance * Worker.SAVINGS_PERCENT * inflation_weightage))
    ...
    if all_emp_workers_acct < 0:
        print("Employed worker low balance")

    if all_emp_workers_acct < 0 and len(unemp_worker_list) > 0:
         print("UnEmployed worker low balance")
    ...
    if current_money_circulation<0:
        print("Money Circulation less than zero")

    if old_money_circulation<0:
        print("Money Circulation less than zero")
    ...
    if current_money_circulation <= old_money_circulation:
        ...
        if isnan(country.product_price):
            print("Is Nan")
