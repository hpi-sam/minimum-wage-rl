...
def start(game, episode_num):
    ...
    game.episode_number = episode_num
    ...
    return collect_metrics(country, num_workers_hired)
...
def collect_metrics(country, num_workers_hired):
    ...
    # current_state["quantity"] = country.quantity/(Quantity_MAX-Quantity_MIN)
    current_state["quantity"] = float("{:.5f}".format(np.log(country.quantity)))
    # 
    # 
 
    current_state["Unemployment Rate"] = float("{:.2f}".format((country.population - num_workers_hired)/country.population))
    # * 100
    #  float("{:.2f}".format(100.0))
    
    current_state["Inflation Rate"] = float("{:.2f}".format(0.0))
    # current_state["Bank balance"] = country.bank.liquid_capital/(Bank_Balance_MAX-Bank_Balance_MIN)
    current_state["Bank balance"] = float("{:.5f}".format(np.log(country.bank.liquid_capital))) 
    # 
    # 
    
    current_state["Poverty Rate"] = float("{:.2f}".format((country.population - num_workers_hired)/country.population))
    #  * 100
    # float("{:.2f}".format(100.0))
    
    # current_state["Money Circulation"] = country.money_circulation/(Money_Circulation_MAX-Money_Circulation_MIN)
    current_state["Money Circulation"] = float("{:.5f}".format(np.log(country.money_circulation)))
    current_state["population"] = float("{:.6f}".format(np.log(country.population)))
    # 
