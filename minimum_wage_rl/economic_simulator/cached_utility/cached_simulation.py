...
def run_market(country, country_companies_list, unemployed_workers_list, game):
    ...
    metrics.game = game
    game.game_metric_list.append(metrics)
    ...
    current_state, normalized_state_values, reward, done, info =  get_current_state_reward(country, metrics)
    ...
    # logging.info("MC - " + str(normalized_state_values[7]) + " - " + str(country.money_circulation))
    ...
    return game, current_state, normalized_state_values, reward, done, info
...
def get_current_state_reward(country, metrics):
    ...
    normalized_state_values, reward = get_game_state(metrics)
    ...
    return current_state, normalized_state_values, float(reward), done, info
...
#     current_state, normalized_state_values, reward, done, info = get_current_state_reward(country, metric)
...
def get_game_state(metric):
    ...
    normalized_state_values.append(metric.minimum_wage)
    normalized_state_values.append(float("{:.6f}".format(np.log(metric.product_price))))
    normalized_state_values.append(float("{:.6f}".format(np.log(metric.produced_quantity))))
    normalized_state_values.append(metric.unemployment_rate/100)
    normalized_state_values.append(metric.inflation)
    normalized_state_values.append(float("{:.6f}".format(np.log(metric.bank_account_balance))))    
    normalized_state_values.append(metric.poverty_rate/100)
    normalized_state_values.append(float("{:.6f}".format(np.log(metric.money_circulation) if metric.money_circulation>0 else metric.money_circulation )))
    normalized_state_values.append(float("{:.6f}".format(np.log(metric.population))))

    reward = calculate_reward(metric)

    return normalized_state_values, reward